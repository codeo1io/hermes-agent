...
| Variable | Description |
|----------|-------------|
| `TERMINAL_ENV` | Backend: `local`, `docker`, `ssh`, `singularity`, `modal`, `daytona`, `vercel_sandbox`. Legacy — prefer `terminal.backend` in `config.yaml` (a stale value here can flip the backend back mid-session) |
