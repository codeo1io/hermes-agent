...
#: Beyond this age a cached doc no longer supplies install pins (they may predate the
#: checkout's own pins) — its kill-list entries still block. Offline-for-days behavior.
LIVE_CATALOG_MAX_STALE_SECONDS = 14 * 24 * 3600
...
def _normalize_repo(url: str) -> str:
    """Canonical comparable form for repo identity: scheme (http≈https≈ssh≈git) and the
    authenticating user are not identity, so every git way of naming the same repo normalizes
    equal — while the host itself stays distinct (a gitlab remote never matches a github entry)."""
    text = url.strip().rstrip("/")
    m = re.match(r"^(?:ssh|git|https?)://(?:[^/@]+@)?([^/]+)(/.*)?$", text, re.IGNORECASE)
    if m:
        text = f"https://{m.group(1)}{m.group(2) or ''}".replace("https://www.", "https://", 1)
    elif "@" in text and ":" in text and not text.startswith(("http://", "https://")):
        # scp-style remote (``git@github.com:owner/repo``)
        host, _, path = text.rpartition(":")
        text = f"https://{host.rsplit('@', 1)[-1]}/{path}"
...
def _live_generated_time(payload: Any) -> float:
    """The doc's ``generated_at`` (ISO-8601 UTC) as epoch seconds; 0.0 when absent/unparseable."""
...
def in_tree_catalog_time() -> float:
    """Newest mtime among in-tree catalog files — ≈ when ``hermes update`` last bumped pins."""
    try:
        return max(p.stat().st_mtime for p in get_catalog_dir().glob("*.yaml"))
    except Exception:
        return 0.0


def _cache_age_seconds() -> float:
    """Age of the on-disk live cache; ``inf`` when there is none."""
    try:
        return time.time() - _live_cache_path().stat().st_mtime
    except Exception:
        return float("inf")


def cached_removed_entries() -> List[RemovedEntry]:
    """Kill list from the on-disk cache alone (no network): a removal published before going
    offline still blocks installs even when the cache is too stale to supply pins."""
    data = _stale_live_cache(_live_cache_path()) or {}
    return _removed_from_list(data.get("removed"))
...
def load_catalog_live() -> List[PluginCatalogEntry]:
    """Pins from the live (or cached) doc, else the in-tree catalog.

    Merge rules (a fresh doc replaces the in-tree LIST; per-entry an in-tree pin wins only when
    the checkout is NEWER than the doc's ``generated_at`` — right after ``hermes update`` a
    pre-bump cache must not resurrect an old sha). A doc older than
    :data:`LIVE_CATALOG_MAX_STALE_SECONDS` supplies no pins at all; its removals still block via
    :func:`live_removed_list`.
    """
    data = fetch_live_catalog()
    in_tree = {e.name: e for e in load_catalog()}
    if data is None:
        return list(in_tree.values())
    if _cache_age_seconds() > LIVE_CATALOG_MAX_STALE_SECONDS:
        return list(in_tree.values())
    in_tree_newer = in_tree_catalog_time() > _live_generated_time(data)
    entries: List[PluginCatalogEntry] = []
    for i, raw in enumerate(data.get("entries", [])):
        entry = entry_from_mapping(raw, f"{LIVE_CATALOG_URL}#{i}")
        if entry is None:
            continue
        competitor = in_tree.get(entry.name)
        if competitor is not None and in_tree_newer:
            entry = competitor
        entries.append(entry)
    return entries
