...
def resolve_external_process_provider_credentials(provider_id: str) -> Dict[str, Any]:
    ...
    if provider_id == "pi-rpc":
        ...
        if not resolved_command:
            raise AuthError(f"Could not find the pi binary '{command}'. Install pi or set HERMES_PI_BIN.", provider=provider_id, code="missing_pi_binary")
        return {"provider": provider_id, "api_key": "pi-rpc", "base_url": "pi://rpc", "command": resolved_command, "args": ["--mode", "rpc", "--no-session"], "source": "process"}
