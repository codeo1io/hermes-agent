...
class TestFTS5Search:
    ...
    def test_search_projection_skips_context_enrichment_queries(self, db, monkeypatch):
        ...
        # Searches may run on a fresh pooled read-only connection per call
        # (WAL read pool), so tracing fixed connections misses the statements.
        # Trace at the seam every read goes through instead.
        statements = []
        ...
        assert context_query_count() == 0  # projection skips enrichment entirely
