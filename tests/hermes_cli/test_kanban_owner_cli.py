...
def test_create_owner_flag_stamps_the_card(run, kanban_home):
    with kbc.connect_closing() as conn:
        assert kb.get_task(conn, _create(run, "--owner", "alice")).owner == "alice"
...
def test_create_json_output_includes_owner(run):
    rc, out, err = run("create", "probe task", "--owner", "alice", "--json")
    assert rc == 0, err
...
def test_list_line_shows_owner_and_json_has_the_key(run):
    ...
    assert any(t["owner"] == "alice" for t in tasks)


def test_list_owner_flag_filters(run):
    tid_a = _create(run, "--owner", "alice")
    _create(run, "--owner", "bob")
    rc, out, err = run("list", "--owner", "alice", "--json")
    assert rc == 0, err
...
def test_owner_verb_unknown_task_fails(run):
    rc, out, err = run("owner", "t_missing", "bob")
    assert rc == 1
