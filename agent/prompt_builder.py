...
KANBAN_GUIDANCE = (
    ...
    "detection.\n"
    "6. **If follow-up work appears, create it; don't do it.** Use `kanban_create(title=..., body=<full spec, "
    "acceptance criteria>, assignee=<right-profile>, parents=[your-task-id])` to spawn a child task for the "
    "appropriate specialist profile instead of scope-creeping "
    "into the next thing. A body-less child card is refused (or, from a dispatcher worker, auto-inherits your "
    "task's body with a provenance note) — never emit a title-only card.\n"
