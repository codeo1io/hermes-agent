...
async def _handle_assist_query_with_llm(ctx: Any, payload: dict[str, Any]) -> dict[str, Any]:
    ...
    Route through Hermes' full local API server so the normal agent loop and
    Home Assistant tools remain available to voice requests.
    """
    import aiohttp
    ...
    api_url = os.getenv(
        "HERMES_ASSIST_API_URL",
        "http://127.0.0.1:8642/v1/chat/completions",
    )
    model = os.getenv("HERMES_ASSIST_MODEL", "glm-5.3")
    headers = {
        "Authorization": f"Bearer {api_key}",
        "Content-Type": "application/json",
    }
    if conversation_id:
        headers["X-Hermes-Session-Id"] = str(conversation_id)
    payload_json = {
        "model": model,
        "model_options": {
            "reasoning_effort": os.getenv("HERMES_ASSIST_REASONING_EFFORT", "low"),
        },
        "messages": [
            {
                "role": "system",
                "content": (
                    ...
                    "Use Home Assistant tools when the request concerns the home. "
                    "Never reveal hidden reasoning, tool internals, or markup."
                ),
            },
            {
                "role": "user",
    ...
    # Keep provider reasoning out of Assist speech when a model serializes it
    # into the visible content instead of the dedicated reasoning field.
    ...
    if "</think>" in response_text.lower():
        response_text = re.split(r"</think>", response_text, maxsplit=1, flags=re.IGNORECASE)[-1]
